...
'`-v .\path\to\volume:/dest`', '`/dev/tty.usbmodem123`' -> '`COM9`').
...
3. ## Generating Subscription Updates
...
```
python -m ectf25.utils.flash .\build_out\max78000.bin COM9
```
...
```
python -m ectf25.tv.list COM9
```

6. ### Subscription Update Tool
...
```
python -m ectf25.tv.subscribe subscription.bin COM9
```
...
```
python -m ectf25.utils.tester --port COM9 -s .\secrets\secrets.json rand -c 1 -f 64
```

9. ## Running the Satellite and Encoder
...
```
python -m ectf25.tv.run localhost 2001 COM9
```
